...
GRANT ALL ON FUTURE DYNAMIC TABLES IN SCHEMA DEV_ML_DB.FEATURES TO ROLE ML_DEV_ROLE;

-- Feature Store specific grants (Dynamic Tables, Tags, Views)
GRANT CREATE DYNAMIC TABLE ON SCHEMA DEV_ML_DB.FEATURES TO ROLE ML_DEV_ROLE;
GRANT CREATE DYNAMIC TABLE ON SCHEMA DEV_ML_DB.PIPELINES TO ROLE ML_DEV_ROLE;
...
GRANT CREATE TABLE ON SCHEMA DEV_ML_DB.FEATURES TO ROLE ML_DEV_ROLE;
...
GRANT CREATE STAGE ON SCHEMA DEV_ML_DB.PIPELINES TO ROLE ML_DEV_ROLE;
...
-- ============================================================================
-- 11. ML_CICD_ROLE DIRECT GRANTS (for EXECUTE AS CALLER procedures)
-- ============================================================================
-- When stored procedures run as EXECUTE AS CALLER, they use the caller's role
-- (ML_CICD_ROLE). Role grants (GRANT ROLE X TO ROLE Y) don't inherit object 
-- privileges automatically. We need direct grants to ML_CICD_ROLE.

-- DEV Environment
GRANT USAGE ON DATABASE DEV_RAW_DB TO ROLE ML_CICD_ROLE;
GRANT USAGE ON SCHEMA DEV_RAW_DB.PUBLIC TO ROLE ML_CICD_ROLE;
GRANT SELECT ON ALL TABLES IN SCHEMA DEV_RAW_DB.PUBLIC TO ROLE ML_CICD_ROLE;
GRANT SELECT ON FUTURE TABLES IN SCHEMA DEV_RAW_DB.PUBLIC TO ROLE ML_CICD_ROLE;
-- Allow ALTER to enable CHANGE_TRACKING for Dynamic Tables (Feature Store requirement)
GRANT ALTER ON ALL TABLES IN SCHEMA DEV_RAW_DB.PUBLIC TO ROLE ML_CICD_ROLE;
GRANT ALTER ON FUTURE TABLES IN SCHEMA DEV_RAW_DB.PUBLIC TO ROLE ML_CICD_ROLE;

GRANT ALL ON DATABASE DEV_ML_DB TO ROLE ML_CICD_ROLE;
GRANT ALL ON ALL SCHEMAS IN DATABASE DEV_ML_DB TO ROLE ML_CICD_ROLE;
GRANT ALL ON ALL TABLES IN DATABASE DEV_ML_DB TO ROLE ML_CICD_ROLE;
GRANT ALL ON ALL STAGES IN DATABASE DEV_ML_DB TO ROLE ML_CICD_ROLE;
GRANT ALL ON ALL DYNAMIC TABLES IN DATABASE DEV_ML_DB TO ROLE ML_CICD_ROLE;
GRANT ALL ON FUTURE TABLES IN SCHEMA DEV_ML_DB.PIPELINES TO ROLE ML_CICD_ROLE;
GRANT ALL ON FUTURE TABLES IN SCHEMA DEV_ML_DB.FEATURES TO ROLE ML_CICD_ROLE;
GRANT ALL ON FUTURE TABLES IN SCHEMA DEV_ML_DB.OUTPUT TO ROLE ML_CICD_ROLE;
GRANT ALL ON FUTURE DYNAMIC TABLES IN SCHEMA DEV_ML_DB.FEATURES TO ROLE ML_CICD_ROLE;
GRANT ALL ON FUTURE STAGES IN SCHEMA DEV_ML_DB.PIPELINES TO ROLE ML_CICD_ROLE;

-- Feature Store specific grants for ML_CICD_ROLE
GRANT CREATE DYNAMIC TABLE ON SCHEMA DEV_ML_DB.FEATURES TO ROLE ML_CICD_ROLE;
GRANT CREATE DYNAMIC TABLE ON SCHEMA DEV_ML_DB.PIPELINES TO ROLE ML_CICD_ROLE;
GRANT CREATE VIEW ON SCHEMA DEV_ML_DB.FEATURES TO ROLE ML_CICD_ROLE;
GRANT CREATE TAG ON SCHEMA DEV_ML_DB.FEATURES TO ROLE ML_CICD_ROLE;
GRANT CREATE TABLE ON SCHEMA DEV_ML_DB.FEATURES TO ROLE ML_CICD_ROLE;
GRANT CREATE TABLE ON SCHEMA DEV_ML_DB.OUTPUT TO ROLE ML_CICD_ROLE;
GRANT CREATE PROCEDURE ON SCHEMA DEV_ML_DB.PIPELINES TO ROLE ML_CICD_ROLE;
GRANT CREATE TASK ON SCHEMA DEV_ML_DB.PIPELINES TO ROLE ML_CICD_ROLE;
GRANT CREATE STAGE ON SCHEMA DEV_ML_DB.PIPELINES TO ROLE ML_CICD_ROLE;

GRANT USAGE ON WAREHOUSE DEV_WH_XS TO ROLE ML_CICD_ROLE;
GRANT OPERATE ON WAREHOUSE DEV_WH_XS TO ROLE ML_CICD_ROLE;

-- SIT Environment
GRANT USAGE ON DATABASE SIT_RAW_DB TO ROLE ML_CICD_ROLE;
GRANT USAGE ON SCHEMA SIT_RAW_DB.PUBLIC TO ROLE ML_CICD_ROLE;
GRANT SELECT ON ALL TABLES IN SCHEMA SIT_RAW_DB.PUBLIC TO ROLE ML_CICD_ROLE;
GRANT SELECT ON FUTURE TABLES IN SCHEMA SIT_RAW_DB.PUBLIC TO ROLE ML_CICD_ROLE;
GRANT ALTER ON ALL TABLES IN SCHEMA SIT_RAW_DB.PUBLIC TO ROLE ML_CICD_ROLE;
GRANT ALTER ON FUTURE TABLES IN SCHEMA SIT_RAW_DB.PUBLIC TO ROLE ML_CICD_ROLE;

GRANT ALL ON DATABASE SIT_ML_DB TO ROLE ML_CICD_ROLE;
GRANT ALL ON ALL SCHEMAS IN DATABASE SIT_ML_DB TO ROLE ML_CICD_ROLE;
GRANT ALL ON ALL TABLES IN DATABASE SIT_ML_DB TO ROLE ML_CICD_ROLE;
GRANT ALL ON ALL STAGES IN DATABASE SIT_ML_DB TO ROLE ML_CICD_ROLE;
GRANT ALL ON ALL DYNAMIC TABLES IN DATABASE SIT_ML_DB TO ROLE ML_CICD_ROLE;
GRANT ALL ON FUTURE TABLES IN SCHEMA SIT_ML_DB.PIPELINES TO ROLE ML_CICD_ROLE;
GRANT ALL ON FUTURE TABLES IN SCHEMA SIT_ML_DB.FEATURES TO ROLE ML_CICD_ROLE;
GRANT ALL ON FUTURE TABLES IN SCHEMA SIT_ML_DB.OUTPUT TO ROLE ML_CICD_ROLE;
GRANT ALL ON FUTURE DYNAMIC TABLES IN SCHEMA SIT_ML_DB.FEATURES TO ROLE ML_CICD_ROLE;
GRANT ALL ON FUTURE STAGES IN SCHEMA SIT_ML_DB.PIPELINES TO ROLE ML_CICD_ROLE;

GRANT USAGE ON WAREHOUSE SIT_WH_S TO ROLE ML_CICD_ROLE;
GRANT OPERATE ON WAREHOUSE SIT_WH_S TO ROLE ML_CICD_ROLE;

-- UAT Environment
GRANT USAGE ON DATABASE UAT_RAW_DB TO ROLE ML_CICD_ROLE;
GRANT USAGE ON SCHEMA UAT_RAW_DB.PUBLIC TO ROLE ML_CICD_ROLE;
GRANT SELECT ON ALL TABLES IN SCHEMA UAT_RAW_DB.PUBLIC TO ROLE ML_CICD_ROLE;
GRANT SELECT ON FUTURE TABLES IN SCHEMA UAT_RAW_DB.PUBLIC TO ROLE ML_CICD_ROLE;
GRANT ALTER ON ALL TABLES IN SCHEMA UAT_RAW_DB.PUBLIC TO ROLE ML_CICD_ROLE;
GRANT ALTER ON FUTURE TABLES IN SCHEMA UAT_RAW_DB.PUBLIC TO ROLE ML_CICD_ROLE;

GRANT ALL ON DATABASE UAT_ML_DB TO ROLE ML_CICD_ROLE;
GRANT ALL ON ALL SCHEMAS IN DATABASE UAT_ML_DB TO ROLE ML_CICD_ROLE;
GRANT ALL ON ALL TABLES IN DATABASE UAT_ML_DB TO ROLE ML_CICD_ROLE;
GRANT ALL ON ALL STAGES IN DATABASE UAT_ML_DB TO ROLE ML_CICD_ROLE;
GRANT ALL ON ALL DYNAMIC TABLES IN DATABASE UAT_ML_DB TO ROLE ML_CICD_ROLE;
GRANT ALL ON FUTURE TABLES IN SCHEMA UAT_ML_DB.PIPELINES TO ROLE ML_CICD_ROLE;
GRANT ALL ON FUTURE TABLES IN SCHEMA UAT_ML_DB.FEATURES TO ROLE ML_CICD_ROLE;
GRANT ALL ON FUTURE TABLES IN SCHEMA UAT_ML_DB.OUTPUT TO ROLE ML_CICD_ROLE;
GRANT ALL ON FUTURE DYNAMIC TABLES IN SCHEMA UAT_ML_DB.FEATURES TO ROLE ML_CICD_ROLE;
GRANT ALL ON FUTURE STAGES IN SCHEMA UAT_ML_DB.PIPELINES TO ROLE ML_CICD_ROLE;

GRANT USAGE ON WAREHOUSE UAT_WH_M TO ROLE ML_CICD_ROLE;
GRANT OPERATE ON WAREHOUSE UAT_WH_M TO ROLE ML_CICD_ROLE;

-- PRD Environment
GRANT USAGE ON DATABASE PRD_RAW_DB TO ROLE ML_CICD_ROLE;
GRANT USAGE ON SCHEMA PRD_RAW_DB.PUBLIC TO ROLE ML_CICD_ROLE;
GRANT SELECT ON ALL TABLES IN SCHEMA PRD_RAW_DB.PUBLIC TO ROLE ML_CICD_ROLE;
GRANT SELECT ON FUTURE TABLES IN SCHEMA PRD_RAW_DB.PUBLIC TO ROLE ML_CICD_ROLE;
GRANT ALTER ON ALL TABLES IN SCHEMA PRD_RAW_DB.PUBLIC TO ROLE ML_CICD_ROLE;
GRANT ALTER ON FUTURE TABLES IN SCHEMA PRD_RAW_DB.PUBLIC TO ROLE ML_CICD_ROLE;

GRANT ALL ON DATABASE PRD_ML_DB TO ROLE ML_CICD_ROLE;
GRANT ALL ON ALL SCHEMAS IN DATABASE PRD_ML_DB TO ROLE ML_CICD_ROLE;
GRANT ALL ON ALL TABLES IN DATABASE PRD_ML_DB TO ROLE ML_CICD_ROLE;
GRANT ALL ON ALL STAGES IN DATABASE PRD_ML_DB TO ROLE ML_CICD_ROLE;
GRANT ALL ON ALL DYNAMIC TABLES IN DATABASE PRD_ML_DB TO ROLE ML_CICD_ROLE;
GRANT ALL ON FUTURE TABLES IN SCHEMA PRD_ML_DB.PIPELINES TO ROLE ML_CICD_ROLE;
GRANT ALL ON FUTURE TABLES IN SCHEMA PRD_ML_DB.FEATURES TO ROLE ML_CICD_ROLE;
GRANT ALL ON FUTURE TABLES IN SCHEMA PRD_ML_DB.OUTPUT TO ROLE ML_CICD_ROLE;
GRANT ALL ON FUTURE DYNAMIC TABLES IN SCHEMA PRD_ML_DB.FEATURES TO ROLE ML_CICD_ROLE;
GRANT ALL ON FUTURE STAGES IN SCHEMA PRD_ML_DB.PIPELINES TO ROLE ML_CICD_ROLE;

GRANT USAGE ON WAREHOUSE PRD_WH_L TO ROLE ML_CICD_ROLE;
GRANT OPERATE ON WAREHOUSE PRD_WH_L TO ROLE ML_CICD_ROLE;
